...
let time_function_precise f ~warmup_iterations ~samples =
  ...
  for _ = 1 to samples do
    ...
    (* Force a small delay between measurements *)
...
let calculate_basic_stats timings =
  ...
  let variance = List.fold timings ~init:0.0 ~f:(fun acc t ->
    let diff = t -. mean in
    acc +. (diff *. diff)) /. Float.of_int count in
...
type variability_stats = {
  mean: float; std_dev: float; variance: float;
  min_time: float; max_time: float; range: float;
  coeff_variation: float; iqr: float; iqr_coeff: float;
  p50: float; p90: float; p95: float; p99: float; p99_9: float;
  p99_p50_ratio: float; p99_9_p50_ratio: float; p90_p10_ratio: float;
  mad: float; mad_coeff: float; rel_variance: float;
}

let calculate_variability_metrics timings =
  let (count, mean, min_time, max_time, std_dev, sorted_timings) = 
    calculate_basic_stats timings in
  ...
  let iqr = p75 -. p25 in
  let iqr_coeff = iqr /. p50 in  (* IQR relative to median *)
  ...
  let mad = List.fold timings ~init:0.0 ~f:(fun acc x -> 
    acc +. Float.abs (x -. p50)) /. Float.of_int count in
  ...
  {
    mean; std_dev; variance; min_time; max_time; range;
    coeff_variation; iqr; iqr_coeff; p50; p90; p95; p99; p99_9;
    p99_p50_ratio; p99_9_p50_ratio; p90_p10_ratio;
    mad; mad_coeff; rel_variance;
  }
...
let variance_ratio_test stack_var heap_var stack_n heap_n =
  ...
  let critical_05 = if Float.(f_stat > 1.2) then "significant at p<0.05" else "not significant" in
  let critical_01 = if Float.(f_stat > 1.5) then "significant at p<0.01" else "not significant" in
...
let levenes_test stack_timings heap_timings =
  let stack_median = 
    ...
    List.nth_exn sorted (List.length sorted / 2) in
  let heap_median = 
    let sorted = List.sort heap_timings ~compare:Float.compare in
    List.nth_exn sorted (List.length sorted / 2) in
    
  (* Calculate absolute deviations from median *)
  let stack_devs = List.map stack_timings ~f:(fun x -> Float.abs (x -. stack_median)) in
  let heap_devs = List.map heap_timings ~f:(fun x -> Float.abs (x -. heap_median)) in
  
  let stack_mean_dev = List.fold stack_devs ~init:0.0 ~f:(+.) /. Float.of_int (List.length stack_devs) in
  let heap_mean_dev = List.fold heap_devs ~init:0.0 ~f:(+.) /. Float.of_int (List.length heap_devs) in
  ...
  let interpretation = 
    ...
    else "No significant difference" in
  (dev_ratio, stack_mean_dev, heap_mean_dev, interpretation)

let analyze_tail_events timings p95 =
  let tail_threshold = p95 *. 2.0 in
  let moderate_threshold = p95 *. 1.5 in  (* Lower threshold for detection *)
  let mild_threshold = p95 *. 1.2 in      (* Even lower threshold *)
  
  let tail_events = List.filter timings ~f:(fun t -> Float.(t > tail_threshold)) in
  let moderate_events = List.filter timings ~f:(fun t -> Float.(t > moderate_threshold)) in
  let mild_events = List.filter timings ~f:(fun t -> Float.(t > mild_threshold)) in
  ...
  let tail_mean = if tail_count > 0 then
    List.fold tail_events ~init:0.0 ~f:(+.) /. Float.of_int tail_count
    else 0.0 
  in
  (tail_threshold, tail_count, tail_mean, moderate_threshold, moderate_count, mild_threshold, mild_count)
...
let print_variability_analysis stack_results heap_results =
  ...
  Stdlib.Printf.printf "                    Stack        Heap         Heap/Stack\n";
  Stdlib.Printf.printf "Std Deviation:      %.6f     %.6f     %.3f\n" 
    stack_stats.std_dev heap_stats.std_dev (heap_stats.std_dev /. stack_stats.std_dev);
  ...
  Stdlib.Printf.printf "Range (max-min):    %.6f     %.6f     %.3f\n"
    stack_stats.range heap_stats.range (heap_stats.range /. stack_stats.range);
  Stdlib.Printf.printf "IQR Coefficient:    %.6f     %.6f     %.3f\n"
    stack_stats.iqr_coeff heap_stats.iqr_coeff (heap_stats.iqr_coeff /. stack_stats.iqr_coeff);
  ...
  Stdlib.Printf.printf "MAD Coefficient:    %.6f     %.6f     %.3f\n"
    stack_stats.mad_coeff heap_stats.mad_coeff (heap_stats.mad_coeff /. stack_stats.mad_coeff);
  Stdlib.Printf.printf "Relative Variance:  %.6f     %.6f     %.3f\n"
    stack_stats.rel_variance heap_stats.rel_variance (heap_stats.rel_variance /. stack_stats.rel_variance);
  
  (* Statistical tests *)
  Stdlib.Printf.printf "\nStatistical Tests:\n";
  let (f_stat, _df1, _df2, crit_05, crit_01) = 
    variance_ratio_test stack_stats.variance heap_stats.variance 
                       (List.length stack_results) (List.length heap_results) in
  Stdlib.Printf.printf "F-test (variance ratio): %.3f (%s, %s)\n" f_stat crit_05 crit_01;
  
  let (dev_ratio, _stack_dev, _heap_dev, interpretation) = 
    levenes_test stack_results heap_results in
  Stdlib.Printf.printf "Levene's test (MAD ratio): %.3f (%s)\n" dev_ratio interpretation;
  
  (* Overall assessment *)
  Stdlib.Printf.printf "\nOverall Assessment:\n";
  let variability_indicators = [
    ("Standard deviation", heap_stats.std_dev /. stack_stats.std_dev);
    ("Coefficient of variation", heap_stats.coeff_variation /. stack_stats.coeff_variation);
  ...
  let higher_variability = List.count variability_indicators ~f:(fun (_, ratio) -> Float.(ratio > 1.1)) in
  ...
  if higher_variability >= total_indicators / 2 then
    Stdlib.Printf.printf "✓ Statistical evidence: Heap allocation has higher timing variability\n"
  else
    Stdlib.Printf.printf "⚠ Insufficient evidence: Timing variability appears similar\n"
...
let print_timing_results function_name =
  ...
  if count > 0 then begin
    let (count, mean, min_time, max_time, std_dev, sorted_timings) = 
      calculate_basic_stats timings in
    let (p50, p90, p95, p99, p99_9) = calculate_percentiles sorted_timings count in
    let (tail_threshold, tail_count, tail_mean, moderate_threshold, moderate_count, mild_threshold, mild_count) = analyze_tail_events timings p95 in
    printf "\n=== %s Timing Analysis ===\n" function_name;
    printf "Total QR codes generated: %d\n" count;
    printf "Total GC events: %d (minor: %d, major: %d)\n" 
      !gc_count !minor_gc_count !major_gc_count;
    ...
    printf "Events exceeding 2x P95 threshold (%.6fs): %d (%.2f%%)\n" 
      tail_threshold tail_count (Float.of_int tail_count /. Float.of_int count *. 100.0);
    printf "Events exceeding 1.5x P95 threshold (%.6fs): %d (%.2f%%)\n" 
      moderate_threshold moderate_count (Float.of_int moderate_count /. Float.of_int count *. 100.0);
    printf "Events exceeding 1.2x P95 threshold (%.6fs): %d (%.2f%%)\n" 
      mild_threshold mild_count (Float.of_int mild_count /. Float.of_int count *. 100.0);
    if tail_count > 0 then
...
let write_results_to_csv function_name =
  let timings = List.rev !timing_results in
  let filename = function_name ^ "_timings.csv" in
  let oc = Out_channel.open_text filename in
  Out_channel.output_string oc "iteration,duration_seconds,tail_event,moderate_event,mild_event\n";
  let count = List.length timings in
  let sorted_timings = List.sort timings ~compare:Float.compare in
  let p95_idx = (count * 95) / 100 in
  let p95 = List.nth_exn sorted_timings p95_idx in
  ...
  List.iteri timings ~f:(fun i duration ->
    ...
    let moderate_flag = if Float.(duration > moderate_threshold) then "1" else "0" in
    let mild_flag = if Float.(duration > mild_threshold) then "1" else "0" in
    Out_channel.output_string oc 
      (Stdlib.Printf.sprintf "%d,%.9f,%s,%s,%s\n" (i + 1) duration tail_flag moderate_flag mild_flag)
  );
  Out_channel.close oc;
  Stdlib.Printf.printf "Timing data written to %s\n" filename

let cleanup_runtime_events cursor =
  Runtime_events.free_cursor cursor

let run_timing_test ~function_name ~qr_function ~test_data ~arena ~num_iterations =
  reset_counters ();
  Stdlib.Printf.printf "\n=== Testing %s ===\n" function_name;
  Stdlib.Printf.printf "Generating %d QR codes...\n" num_iterations;
  (* Pre-allocate and warm up *)
  Stdlib.Printf.printf "Warming up (50 iterations)...\n";
  for _ = 1 to 50 do
    let _ = qr_function arena test_data Config.ECL.M in ()
  done;
  (* Force GC to start with clean state *)
  Stdlib.Gc.full_major ();
  Stdlib.Gc.compact ();
  let (cursor, callbacks) = setup_runtime_events () in
  Stdlib.Printf.printf "Starting measurements...\n";
  for i = 1 to num_iterations do
    if i % 100 = 0 then Stdlib.Printf.printf "Progress: %d/%d\r%!" i num_iterations;
    (* Take 3 samples per iteration and use the minimum (reduces noise) *)
    let measurements = time_function_precise (fun () -> qr_function arena test_data Config.ECL.M) 
                        ~warmup_iterations:1 ~samples:3 in
    let min_measurement = List.fold measurements ~init:Float.max_value ~f:Float.min in
  ...
  let _ = Runtime_events.read_poll cursor callbacks None in
  cleanup_runtime_events cursor;
  Stdlib.Printf.printf "\nGeneration complete!\n";
  print_timing_results function_name;
  write_results_to_csv function_name;
  !timing_results

let compare_functions test_data arena num_iterations =
  Stdlib.Printf.printf "=== QR Generation Function Comparison ===\n";
  Stdlib.Printf.printf "Test parameters: %d iterations, %d character strings\n" 
    num_iterations (String.length test_data);
  (* Test generate_qr_stack *)
  let stack_results = run_timing_test 
    ~function_name:"generate_qr_stack"
    ~qr_function:Encoding.generate_qr_stack
    ~test_data
    ~arena
    ~num_iterations in
  (* Test generate_qr *)
  let heap_results = run_timing_test
    ~function_name:"generate_qr_heap"
    ~qr_function:(fun arena data ecl -> let _ = Encoding.generate_qr arena data ecl in ())
    ~test_data
    ~arena
    ~num_iterations in
  (* Compare results *)
  let stack_mean = List.fold stack_results ~init:0.0 ~f:(+.) /. Float.of_int (List.length stack_results) in
  let heap_mean = List.fold heap_results ~init:0.0 ~f:(+.) /. Float.of_int (List.length heap_results) in
  let speedup = heap_mean /. stack_mean in
  Stdlib.Printf.printf "\n=== Performance Comparison ===\n";
  Stdlib.Printf.printf "Stack-based mean: %.6f seconds\n" stack_mean;
  Stdlib.Printf.printf "Heap-based mean:  %.6f seconds\n" heap_mean;
  Stdlib.Printf.printf "Stack speedup:    %.2fx\n" speedup;
  if Float.(speedup > 1.0) then
    Stdlib.Printf.printf "✓ Stack-based implementation is faster\n"
  else
    Stdlib.Printf.printf "⚠ Heap-based implementation is faster\n";
  
  (* Add detailed statistical variability analysis *)
  print_variability_analysis stack_results heap_results

let () =
  let str_len = 500 in  (* Much larger strings to create more allocation pressure *)
  let num_iterations = 10000 in  (* Reduced for faster analysis *)
  let test_data = random_alphanumeric_string str_len in
  (* Determine the correct QR version for our test data *)
  ...
  Stdlib.Printf.printf "Starting QR generation timing analysis...\n";
  compare_functions test_data arena num_iterations
