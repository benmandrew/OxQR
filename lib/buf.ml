...
let add_bits t value count =
  ...
  let maxv = if count = 31 then Int32.to_int Int32.max_int else (1 lsl count) - 1 in
...
let add_byte t v =
  ...
  if t.cur_bits_filled = 0 then (
    ...
    if t.len_bytes >= t.cap_bytes then invalid_arg "Bit_buffer.add_byte: overflow";
    flush_full_byte t v
  ) else (
    (* Add 8 bits MSB-first. *)
    add_bits t v 8
  )
...
let add_bytes t bs =
  if t.cur_bits_filled = 0 then (
    ...
    if t.len_bytes + n > t.cap_bytes then invalid_arg "Bit_buffer.add_bytes: overflow";
    Bytes.blit bs 0 t.buf t.len_bytes n;
    t.len_bytes <- t.len_bytes + n;
    t.len_bits <- t.len_bits + (n * 8)
  ) else (
    (* Not aligned: add byte-by-byte. *)
