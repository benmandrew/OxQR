...
let[@zero_alloc] create len_bytes = exclave_
...
let[@zero_alloc] write_bit (t @ local) b = exclave_
  check_space t 1
  |> Result.map ~f:(stack_ fun () ->
...
let[@zero_alloc] write_bits_msb (t @ local) ~value ~width =
...
let[@zero_alloc] write_byte (t @ local) b =
  ...
  exclave_ write_bits_msb t ~value:b ~width:8
