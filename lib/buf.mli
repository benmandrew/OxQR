...
(** [create ~bytes] preallocates exactly [bytes] capacity; writing past it raises [Invalid_argument]. *)
...
(** [add_bits t value count] appends the [count] most-significant bits of
    [value], MSB-first. Requires [0 <= count <= 31] and [value] fits in [count] bits. Raises if capacity is exceeded. *)
...
(** Snapshot the contents as bytes. If there is a partially-filled byte,
    it is padded with 0s in the least-significant bits. Does not modify [t]. *)
